//Derived from: https://github.com/Nanode/Webserver-Live-Thermistor-Reading and http://www.arduino.cc/playground/ComponentLib/Thermistor and https://github.com/dingram/Nanode/blob/master/Ethershield_webserver/Ethershield_webserver.pde
#include "EtherShield.h"
#include <math.h>
#define TempPIN 0                       // Analog Pin 0
/* old Thermistor stuff                                        // set to the measured Vcc.
...
static uint8_t mymac[6] = {0x00,0x04,0xA3,0x2C,0x29,0x1C };
...
uint16_t print_webpage(uint8_t *buf)
{
  uint16_t plen;  
  plen=http200ok();
  plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("<html><head><title>Nanode temperature sensor</title></head><body>"));
  plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("<center><h1>Nanode temperature sensor</h1>"));
...
return (analogRead(pin) * 0.00322265625); //converting from a 0 to 1023 digital range
...
void loop(){
  ...
  /* old thermistor code
    ...
    */
    temp = getVoltage(TempPIN);  //getting the voltage reading from TMP36 the temperature sensor
    temp = (temp - .5) * 100;    //converting from 10 mv per degree with 500 mV offset
